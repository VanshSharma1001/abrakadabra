...
def get_db():
    ...
    try:
        ...
        # If URI specifies a default DB, get_database() uses it; otherwise fallback to 'citytracker'
        try:
...
# Attempt seeding; ignore errors but show nothing sensitive
try:
    _created = seed_default_accounts()
    if _created:
        # harmless hint for local debugging (will appear in URL)
        st.experimental_set_query_params(seeded=",".join(_created))
except Exception:
    pass
...
if not st.session_state.logged_in:
    ...
    st.write("Tip: This app auto-seeds demo accounts on first run. You can log in with:")
