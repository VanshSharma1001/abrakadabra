...
with col2:
    ...
    if not issues:
        ...
        # --- ADMIN CSV EXPORT BUTTON (visible only to admins) ---
        if user["role"] == "admin":
            try:
                csv_bytes = df.to_csv(index=False).encode("utf-8")
                filename = f"issues_export_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
                st.download_button(
                    label="📥 Export visible issues to CSV",
                    data=csv_bytes,
                    file_name=filename,
                    mime="text/csv"
                )
            except Exception as e:
                # don't crash UI on export error; show small message
                st.error(f"Failed to prepare CSV export: {e}")
        # --- end admin export button ---
