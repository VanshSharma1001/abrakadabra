...
# --------------------
def gen_salt(length: int = 16):
    return base64.b64encode(pysecrets.token_bytes(length)).decode("utf-8")

def hash_password(password: str, salt: str):
    """Return a hex digest for password + salt using PBKDF2-HMAC-SHA256"""
    # pbkdf2_hmac requires bytes
...
def get_db():
    try:
        ...
        st.error("Mongo URI not found in st.secrets. Please add it to .streamlit/secrets.toml")
        st.stop()

    client = MongoClient(mongo_uri)
    db = client.get_database()  # uses the default DB from the URI or the admin DB
    return db
...
# If not logged in, show a friendly message and stop main UI
...
# Two-column layout: left = input, right = output/listing
...
with col2:
    ...
    if user["role"] == "admin":
        ...
        query = {"reporter_username": user["username"]}
    ...
    if f_search:
        # simple text search across title and location (case-insensitive)
        query["$or"] = [
    ...
    # Fetch issues, latest first
    cursor = issues_col.find(query).sort("created_at", -1).limit(200)
    issues = list(cursor)

    if not issues:
        st.info("No issues found for these filters.")
    else:
        # Convert to dataframe for a compact table
        def doc_to_row(d):
            return {
                "id": str(d.get("_id")),
                "title": d.get("title"),
        ...
        # Detailed controls for each issue (admin can change status)
        st.markdown("---")
        ...
        if st.button("Load issue"):
            issue_doc = issues_col.find_one({"_id": None})  # placeholder to satisfy type-checker
            try:
                from bson import ObjectId
                oid = ObjectId(sel_id.strip())
                issue_doc = issues_col.find_one({"_id": oid})
            except Exception as e:
                st.error("Invalid id or not found. Make sure you copied the full id from the table above.")
                issue_doc = None
            ...
            if issue_doc:
                ...
                # Comments / status update (both admin and reporter allowed)
                st.markdown("### Add admin note / change status")
                with st.form("manage_form"):
                    note = st.text_area("Add note (optional)")
                    new_status = st.selectbox("Set status", ("open", "in-progress", "closed"), index=("open","in-progress","closed").index(issue_doc.get("status")))
                    submit_manage = st.form_submit_button("Save updates")
                if submit_manage:
                    update_data = {"updated_at": datetime.utcnow()}
                    if note:
                        # append note to a 'notes' array
                        update_data["$push"] = {"notes": {"author": user["username"], "text": note, "time": datetime.utcnow()}}
                    # set status
                    issues_col.update_one({"_id": issue_doc["_id"]}, {"$set": {"status": new_status, "updated_at": datetime.utcnow()}, **({"$push": {"notes": {"author": user["username"], "text": note, "time": datetime.utcnow()}}} if note else {})})
